  ...
  public function index(Request $request)
{
    ...
    // Récupérer uniquement 39 questions triées par 'numero'
    $questions = ComprehensionEcrite::orderBy('numero')->take(39)->get();
    ...
    $title = 'Expohub | test-TCF CE';
    
    if ($questions->isEmpty()) {
        return view('test.indisponible', [
            'test' => 'Comprehension Ecrite',
            'title'=> 'Expohub | indisponible',
        ]);
    }

    return view('test.comprehension_ecrite', compact('questions', 'test_type', 'reponses','title'));
}
    ...
    public function enregistrerResultatFinal(Request $request)
    {
        ...
        Niveau::updateOrCreate(
            [
                'user_id' => $userId,
                'test_type' => $request->abonnement_id,
            ],
            [
   ...
   /**
 * Détermine le niveau de l'utilisateur selon le nombre de bonnes réponses.
 */
private function determineNiveau(int $nbBonnesReponses): string
{
    return match (true) {
        $nbBonnesReponses >= 1 && $nbBonnesReponses <= 5 => 'A1', // Élémentaire débutant : reconnaissance de mots et phrases simples
        $nbBonnesReponses >= 6 && $nbBonnesReponses <= 15 => 'A2', // Élémentaire avancé : communication simple sur des sujets familiers
        $nbBonnesReponses >= 16 && $nbBonnesReponses <= 20 => 'B1', // Niveau de base : peut gérer des situations courantes
        $nbBonnesReponses >= 21 && $nbBonnesReponses <= 25 => 'B2', // Intermédiaire : discours clair sur divers sujets
        $nbBonnesReponses >= 26 && $nbBonnesReponses <= 33 => 'C1', // Avancé : expression fluide et structurée
        $nbBonnesReponses >= 34 && $nbBonnesReponses <= 39 => 'C2', // Très avancé : maîtrise complète de la langue
        default => 'A0', // Aucun résultat ou niveau non défini
    };
}
    ...
    public function resultat()
    {
        ...
        ->select(
            ...
            'comprehension_ecrite.propositions',
    ...
    $reponses = $reponses->map(function ($item) {
        // Décoder les propositions JSON
        $propositions = is_array($item->propositions)
            ? $item->propositions
            : json_decode($item->propositions, true);

        $mapping = ['A' => 0, 'B' => 1, 'C' => 2, 'D' => 3];

        // ✅ Corrigé ici : on utilise bien bonne_reponse et non reponse
        $bonneIndex = $mapping[$item->bonne_reponse] ?? null;
        $item->bonne_reponse_texte = $bonneIndex !== null ? ($propositions[$bonneIndex] ?? null) : null;

        $userIndex = $mapping[$item->reponse_utilisateur] ?? null;
        $item->reponse_utilisateur_texte = $userIndex !== null ? ($propositions[$userIndex] ?? null) : null;
        ...
        $title='Expohub | test-TCF CE-resultat';
