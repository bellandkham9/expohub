...
<div class="container-fluid px-4">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h1 class="fw-bold text-gradient">
            <i class="fas fa-robot me-2"></i>Générateur de Contenu Pédagogique
        </h1>
                ...
                <div class="tab-pane fade show active" id="ecrite" role="tabpanel">
                    ...
                    <div class="recent-item mb-3 p-3 bg-light rounded">
                        <div class="d-flex justify-content-between">
                            ...
                            <span class="badge bg-primary">{{ $q->created_at->diffForHumans() }}</span>
                        </div>
                ...
                <div class="tab-pane fade" id="orale" role="tabpanel">
                    ...
                    <div class="recent-item mb-3 p-3 bg-light rounded">
                        <div class="d-flex justify-content-between">
                            ...
                            <span class="badge bg-success">{{ $q->created_at->diffForHumans() }}</span>
                        </div>
                ...
                <div class="tab-pane fade" id="expression-ecrite" role="tabpanel">
                    ...
                    <div class="recent-item mb-3 p-3 bg-light rounded">
                        <div class="d-flex justify-content-between">
                            ...
                            <span class="badge bg-info">{{ $t->created_at->diffForHumans() }}</span>
                        </div>
                ...
                <div class="tab-pane fade" id="expression-orale" role="tabpanel">
                    ...
                    <div class="recent-item mb-3 p-3 bg-light rounded">
                        <div class="d-flex justify-content-between">
                            ...
                            <span class="badge bg-warning text-dark">{{ $t->created_at->diffForHumans() }}</span>
                        </div>
...
document.addEventListener("DOMContentLoaded", function() {
  ...
  @if(session('success') && session('generated_section') === 'comprehension_ecrite')
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            let audios = {!! json_encode(session('generated_audios') ?? []) !!};
            if (audios.length > 0) {
                let htmlContent = '<div class="text-start" style="max-height: 400px; overflow-y: auto;">';
                audios.forEach((a, i) => {
                    htmlContent += `
   ...
   @if(session('success') && session('generated_section') === 'expression_ecrite')
    Swal.fire({
        ...
        if (result.isConfirmed) {
            let taches = {!! json_encode(session('generated_taches') ?? []) !!};
            if (taches.length > 0) {
                let htmlContent = '<div class="text-start" style="max-height: 400px; overflow-y: auto;">';
                taches.forEach((t, i) => {
                    htmlContent += `
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            let taches = {!! json_encode(session('generated_taches_orales') ?? []) !!};
            if (taches.length > 0) {
                let htmlContent = '<div class="text-start" style="max-height: 400px; overflow-y: auto;">';
                taches.forEach((t, i) => {
                    htmlContent += `
