...
<head>
    ...
    <!-- Consolidation des ressources CSS -->
    ...
    <style>
        ...
        :root {
            ...
            --light-success: rgba(25, 135, 84, 0.1);
            --light-info: rgba(13, 202, 240, 0.1);
            --light-warning: rgba(255, 193, 7, 0.1);
        }
...
<body>
    <div class="container-fluid">
        <div class="row">
            ...
            <div class="col-md-9 col-lg-10 main-content">
                <div class="m-4">
                    <h2>Géneration de nouvelles données</h2>
                </div>
                                    ...
                                    @foreach ($recent_ecrite as $q)
                                        <div class="recent-item mb-3 p-3 bg-light rounded">
                                            <div class="d-flex justify-content-between">
                                                ...
                                                <span class="badge-timestamp bg-primary">{{ $q->created_at->diffForHumans() }}</span>
                                            </div>
                                    ...
                                    @foreach ($recent_orale as $q)
                                        <div class="recent-item mb-3 p-3 bg-light rounded">
                                            <div class="d-flex justify-content-between">
                                                ...
                                                <span class="badge-timestamp bg-success">{{ $q->created_at->diffForHumans() }}</span>
                                            </div>
                                    ...
                                    @foreach ($recent_expression_ecrite as $t)
                                        <div class="recent-item mb-3 p-3 bg-light rounded">
                                            <div class="d-flex justify-content-between">
                                                ...
                                                <span class="badge-timestamp bg-info">{{ $t->created_at->diffForHumans() }}</span>
                                            </div>
                                    ...
                                    @foreach ($recent_expression_orale as $t)
                                        <div class="recent-item mb-3 p-3 bg-light rounded">
                                            <div class="d-flex justify-content-between">
                                                ...
                                                <span class="badge-timestamp bg-warning">{{ $t->created_at->diffForHumans() }}</span>
                                            </div>
    ...
    <!-- Scripts consolidés en bas de page pour une meilleure performance -->
    ...
    <script>
        ...
        const AlertUtils = {
            showSuccess: function(title, html, confirmText, cancelText, backdrop, btnClass) {
                return Swal.fire({
                    icon: 'success',
                    title: title,
                    html: html,
                    ...
                    backdrop: backdrop,
                    customClass: {
                        confirmButton: `btn ${btnClass} px-4 py-2`,
            ...
            showError: function(message) {
                return Swal.fire({
                    ...
                    background: '#f8f9fa',
                    backdrop: `rgba(220,53,69,0.4) url("/images/error.gif") center top no-repeat`,
                    customClass: {
        ...
        const FormUtils = {
            setLoadingState: function(button, isLoading) {
                ...
                if (isLoading) {
                    ...
                    button.innerHTML = button.innerHTML.replace(/Générer.*/, 'Génération en cours...');
                } else {
                    button.disabled = false;
                    // Restaurer l'icône et le texte d'origine selon le type de bouton
                    if (button.id === 'btnGenerateEcrite') {
                        if (icon) icon.className = 'fas fa-magic me-2';
                        button.innerHTML = '<i class="fas fa-magic me-2"></i>Générer des questions';
                    }
                    // Ajouter les autres conditions pour les autres boutons
                }
            }
        };
        ...
        document.addEventListener("DOMContentLoaded", function() {
            ...
            @if (session('success'))
                const section = '{{ session('generated_section') }}';
                const message = `{!! session('success') !!}`;
                const data = {!! json_encode(session()->all()) !!};
                
                switch(section) {
                    case 'comprehension_ecrite':
                        handleComprehensionEcriteSuccess(message, data.generated_questions || []);
                        break;
                    case 'comprehension_orale':
                        handleComprehensionOraleSuccess(message, data.generated_audios || []);
                        break;
                    case 'expression_ecrite':
                        handleExpressionEcriteSuccess(message, data.generated_taches || []);
                        break;
                    case 'expression_orale':
                        handleExpressionOraleSuccess(message, data.generated_taches_orales || []);
                        break;
                }
            @elseif (session('error'))
                const errorSection = '{{ session('generated_section') }}';
            ...
            document.querySelectorAll('.generate-btn').forEach(btn => {
                btn.addEventListener('click', function(e) {
                    ...
                    FormUtils.setLoadingState(this, true);
                    form.submit();
                    
                    // Timeout de sécurité pour réactiver le bouton après 5s
                    setTimeout(() => FormUtils.setLoadingState(btn, false), 5000);
                });
            });
        ...
        // Handlers pour les différents types de succès
        function handleComprehensionEcriteSuccess(message, questions) {
            AlertUtils.showSuccess(
                'Questions générées avec succès!',
                message,
                "<i class='fas fa-list-ol me-2'></i>Voir les questions",
                "<i class='fas fa-times me-2'></i>Fermer",
                `rgba(0,123,255,0.4) url("/images/confetti.gif") center top no-repeat`,
                'btn-primary'
            ).then((result) => {
                if (result.isConfirmed && questions.length > 0) {
                    showQuestionsPopup(questions);
                }
            });
        }

        function handleComprehensionOraleSuccess(message, audios) {
            AlertUtils.showSuccess(
                'Audios générés avec succès!',
                message,
                "<i class='fas fa-headphones me-2'></i>Écouter",
                "<i class='fas fa-times me-2'></i>Fermer",
                `rgba(40,167,69,0.4) url("/images/audio-wave.gif") center top no-repeat`,
                'btn-success'
            ).then((result) => {
                if (result.isConfirmed && audios.length > 0) {
                    showAudiosPopup(audios);
                }
            });
        }

        function handleExpressionEcriteSuccess(message, taches) {
            AlertUtils.showSuccess(
                'Tâches générées avec succès!',
                message,
                "<i class='fas fa-eye me-2'></i>Voir les tâches",
                "<i class='fas fa-times me-2'></i>Fermer",
                `rgba(23,162,184,0.4) url("/images/writing.gif") center top no-repeat`,
                'btn-info'
            ).then((result) => {
                if (result.isConfirmed && taches.length > 0) {
                    showTachesPopup(taches, 'ecrite');
                }
            });
        }

        function handleExpressionOraleSuccess(message, taches) {
            AlertUtils.showSuccess(
                'Tâches générées avec succès!',
                message,
                "<i class='fas fa-play-circle me-2'></i>Écouter",
                "<i class='fas fa-times me-2'></i>Fermer",
                `rgba(255,193,7,0.4) url("/images/speaking.gif") center top no-repeat`,
                'btn-warning'
            ).then((result) => {
                if (result.isConfirmed && taches.length > 0) {
                    showTachesPopup(taches, 'orale');
                }
            });
        }

        // Fonctions pour afficher les popups de contenu généré
        function showQuestionsPopup(questions) {
        ...
        function showAudiosPopup(audios) {
            let htmlContent = '<div class="text-start" style="max-height: 400px; overflow-y: auto;">';
            audios.forEach((a, i) => {
                htmlContent += `
        ...
        function showTachesPopup(taches, type) {
            const isOral = type === 'orale';
            const title = isOral ? 
                '<i class="fas fa-microphone text-warning me-2"></i>Tâches Générées' : 
                '<i class="fas fa-keyboard text-info me-2"></i>Tâches Générées';
            
            const bgClass = isOral ? 'bg-light-warning' : 'bg-light-info';
            const iconClass = isOral ? 'fas fa-comment-dots text-warning' : 'fas fa-pen-fancy text-info';
            
            let htmlContent = '<div class="text-start" style="max-height: 400px; overflow-y: auto;">';
            
            taches.forEach((t, i) => {
                htmlContent += `
                    <div class="mb-3 p-3 ${bgClass} rounded">
                        <h6 class="fw-bold"><i class="${iconClass} me-2"></i>Tâche ${i+1}</h6>
                `;
                ...
                <div class="container-fluid px-4">
                    ...
                    <div class="row g-4">
                        ...
                        <div class="col-lg-6">
                            <div class="card generation-card h-100 border-start border-5 border-primary">
                                ...
                                <div class="card-body">
                                    <form method="POST" action="{{ route('train.ce.generate') }}"
                        ...
                        <div class="col-lg-6">
                            <div class="card generation-card h-100 border-start border-5 border-success">
                                ...
                                <div class="card-body">
                                    <form method="POST" action="{{ route('train.co.generate') }}"
                                        class="generation-form">
                                        @csrf
                                        <div class="mb-3">
                                            <label class="form-label fw-bold">
                                        ...
                                        <button type="button" class="btn btn-success w-100 generate-btn"
                                            id="btnGenerateOrale">
                                            <i class="fas fa-wave-square me-2"></i>Générer des audios
                                        </button>
                                    </form>
                        ...
                        <div class="col-lg-6">
                            <div class="card generation-card h-100 border-start border-5 border-info">
                                ...
                                <div class="card-body">
                                    <form action="{{ route('train.ee.generate') }}" method="POST"
                                        class="generation-form">
                                        @csrf
                                        <div class="mb-3">
                                            <label class="form-label fw-bold">
                                            ...
                                            <div class="input-group">
                                                <span class="input-group-text"><i class="fas fa-hashtag"></i></span>
                                                <input type="number" name="nb_taches" class="form-control"
                                                    value="1" min="1" max="10" required>
                                            </div>
                                        </div>
                                        <button type="button" class="btn btn-info w-100 generate-btn"
                                            id="btnGenerateEcrit">
                                            <i class="fas fa-keyboard me-2"></i>Générer des tâches
                                        </button>
                                    </form>
                        ...
                        <div class="col-lg-6">
                            <div class="card generation-card h-100 border-start border-5 border-warning">
                                ...
                                <div class="card-body">
                                    <form action="{{ route('train.eo.generate') }}" method="POST"
                                        class="generation-form">
                                        @csrf
                                        <div class="mb-3">
                                            <label class="form-label fw-bold">
                                        ...
                                        <button type="button" class="btn btn-warning w-100 generate-btn"
                                            id="btnGenerateOral">
                                            <i class="fas fa-microphone me-2"></i>Générer des tâches
                                        </button>
                                    </form>
                    ...
                    <div class="card mt-4 shadow-sm">
                        ...
                        <div class="card-body">
                            <ul class="nav nav-tabs" id="recentContentTabs" role="tablist">
                                <li class="nav-item" role="presentation">
                                    <button class="nav-link active" id="ecrite-tab" data-bs-toggle="tab"
                                        data-bs-target="#ecrite" type="button" role="tab">
                                        <i class="fas fa-book-reader me-2"></i>Écrite
                                    </button>
                                </li>
                                <li class="nav-item" role="presentation">
                                    <button class="nav-link" id="orale-tab" data-bs-toggle="tab"
                                        data-bs-target="#orale" type="button" role="tab">
                                        <i class="fas fa-microphone-alt me-2"></i>Orale
                                    </button>
                                </li>
                                <li class="nav-item" role="presentation">
                                    <button class="nav-link" id="expression-ecrite-tab" data-bs-toggle="tab"
                                        data-bs-target="#expression-ecrite" type="button" role="tab">
                                        <i class="fas fa-pen-fancy me-2"></i>Expression Écrite
                                    </button>
                                </li>
                                <li class="nav-item" role="presentation">
                                    <button class="nav-link" id="expression-orale-tab" data-bs-toggle="tab"
                                        data-bs-target="#expression-orale" type="button" role="tab">
                                        <i class="fas fa-comment-dots me-2"></i>Expression Orale
                                    </button>
                                </li>
                            </ul>
                            <div class="tab-content p-3 border border-top-0 rounded-bottom"
                                id="recentContentTabContent">
                                <div class="tab-pane fade show active" id="ecrite" role="tabpanel">
                                    @foreach ($recent_ecrite as $q)
                                        <div class="recent-item mb-3 p-3 bg-light rounded">
                                            <div class="d-flex justify-content-between">
                                                ...
                                                <span class="bg-primary"
                                                    style="padding: 10px; color: white; border-radius: 15px; box-shadow: 2px 2px 2px 2px gainsboro;">{{ $q->created_at->diffForHumans() }}</span>
                                            </div>
                                            <p class="text-muted small mb-0">{{ $q->situation }}</p>
                                        </div>
                                    @endforeach
                                ...
                                <div class="tab-pane fade" id="orale" role="tabpanel">
                                    @foreach ($recent_orale as $q)
                                        <div class="recent-item mb-3 p-3 bg-light rounded">
                                            <div class="d-flex justify-content-between">
                                                <h6 class="fw-bold">
                                                    {{ Str::limit(strip_tags($q->question_audio ?? ''), 50, '...') }}
                                                </h6>
                                                <span class="bg-success"
                                                    style="padding: 10px; color: white; border-radius: 15px; box-shadow: 2px 2px 2px 2px gainsboro;">{{ $q->created_at->diffForHumans() }}</span>

                                            </div>
                                            <audio controls src="{{ asset('storage/' . $q->question_audio) }}"
                                    ...
                                    @foreach ($recent_expression_ecrite as $t)
                                        <div class="recent-item mb-3 p-3 bg-light rounded">
                                            <div class="d-flex justify-content-between">
                                                <h6 class="fw-bold">Tâche #{{ $t->numero_tache }}</h6>
                                                <span class="bg-info"
                                                    style="padding: 10px; color: white; border-radius: 15px; box-shadow: 2px 2px 2px 2px gainsboro;">{{ $t->created_at->diffForHumans() }}</span>
                                            </div>
                                            <p class="mb-1"><strong>Contexte:</strong>
                                                {{ Str::limit($t->contexte_texte, 70) }}</p>
                                            <p class="mb-0"><strong>Consigne:</strong>
                                                {{ Str::limit($t->consigne, 70) }}</p>
                                        </div>
                                    @endforeach
                                </div>
                                <div class="tab-pane fade" id="expression-orale" role="tabpanel">
                                    @foreach ($recent_expression_orale as $t)
                                        <div class="recent-item mb-3 p-3 bg-light rounded">
                                            ...
                                            <p class="mb-1"><strong>Type:</strong> {{ $t->type }}</p>
                                            <p class="mb-1"><strong>Contexte:</strong>
                                                {{ Str::limit($t->contexte, 50) }}</p>
                                            @if ($t->consigne_audio)
                                                <audio controls src="{{ asset('storage/' . $t->consigne_audio) }}"
                                                    class="w-100 mt-1"></audio>
                                            @endif
                                        </div>
                                    @endforeach
                                </div>
    ...
    : '<p class="text-muted">Aucun audio généré</p>'
            ...
            document.querySelectorAll('.generate-btn').forEach(btn => {
                btn.addEventListener('click', function(e) {
                    ...
                    setTimeout(() => {
                        this.disabled = false;
                        if (icon) icon.className = originalText.match(/fa-[^\s]+/)[0] +
                            ' me-2';
